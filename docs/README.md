...
| [Scheduled, headerless intercepts](SCHEDULES.md) | The one mode that is not header-keyed: a declared window, a whole workload or one DNS name diverted for its duration, forcing a window on demand, why the two intercept modes cannot share a workload, and what a global intercept costs when it dies |
