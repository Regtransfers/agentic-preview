...
// ScheduleSpec declares one workload that is globally intercepted for a
// recurring window and left completely alone outside it.
type ScheduleSpec struct {
	// Name identifies the schedule. It is used as the work id this intercept
	// is registered and reported under, so it must be shaped like one, and it
	// is RESERVED: a POST /previews naming it is refused rather than allowed to
	// share a registry entry with the schedule.
	...
	// Windows are the recurring spans the intercept is up for. Several are
	// allowed and they may overlap; the intercept is up if any of them covers
	// the moment.
...
func compileSchedule(spec ScheduleSpec, defLoc string, cfg *config) (*schedule, error) {
	...
	if spec.Name == "" {
		return nil, fmt.Errorf("name is required: it is the id this intercept is reported under")
	}
	...
	if spec.Workload == "" || spec.Namespace == "" {
		return nil, fmt.Errorf("workload and namespace are both required")
	}
	if !nameRE.MatchString(spec.Workload) {
		return nil, fmt.Errorf("workload %q must be a DNS label", spec.Workload)
	}
	...
	if svc := strings.TrimSpace(spec.Service); svc != "" {
		if !nameRE.MatchString(svc) {
			return nil, fmt.Errorf("service %q must be a DNS label", svc)
		}
		spec.Service = svc
	}
	...
	if svc == "" {
		return nil, fmt.Errorf("targetService is required: the Service traffic is diverted to while the window is open")
	}
	...
	if !nameRE.MatchString(svc) {
		return nil, fmt.Errorf("targetService %q must be a DNS label, optionally as name.namespace", spec.TargetService)
	}
	if !nameRE.MatchString(svcNS) {
		return nil, fmt.Errorf("target namespace %q must be a DNS label", svcNS)
	}
	...
	if port < 1 || port > 65535 {
		return nil, fmt.Errorf("targetPort %d is not a port", port)
	}
	...
	if !nameRE.MatchString(name) {
		return nil, fmt.Errorf("workload %q and name %q produce an unusable intercept name %q",
			spec.Workload, spec.Name, name)
	}

	return &schedule{
		spec: spec, interceptName: name,
		targetService: svc, targetNamespace: svcNS, targetPort: port, portID: portID,
		loc: l, windows: ws,
	}, nil
}
